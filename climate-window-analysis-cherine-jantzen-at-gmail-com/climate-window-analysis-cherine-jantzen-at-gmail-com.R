...
first_window_Qrobur_file <- here::here("data", "climwin_outputs_Qrobur.rda")
save(first_window_Qrobur, file = first_window_Qrobur_file)

first_window_Qrubra_file <- here::here("data", "climwin_outputs_Qrubra.rda")
save(first_window_Qrubra, file = first_window_Qrubra_file)



# capturing outputs
file <- file(paste0('/tmp/first_window_Qrobur_file_', id, '.json'))
writeLines(toJSON(first_window_Qrobur_file, auto_unbox=TRUE), file)
close(file)
file <- file(paste0('/tmp/first_window_Qrubra_file_', id, '.json'))
writeLines(toJSON(first_window_Qrubra_file, auto_unbox=TRUE), file)
close(file)
